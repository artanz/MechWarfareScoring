...
class MWScoreFrame( wx.Frame ):
        ...
        def TransponderVar( self, event ):
                ...
                if dlg.ShowModal() == wx.ID_OK:
                        ...
                        if not self.ScoreServer.TransponderListener.Xbee:
                                wx.MessageBox("Could not open Transponder port " + dlg.PortChoice.GetValue(), "Error", wx.OK | wx.ICON_ERROR)
                dlg.Destroy()
...
class MatchPanel( wx.Panel ):
        ...
        def __init__( self, parent, id ):
                ...
                for m in range(len(self.MechList)):
